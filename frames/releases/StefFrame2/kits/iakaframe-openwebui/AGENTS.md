# AGENTS.md — contrat de travail iakaframe (incarnation Open WebUI)

> Contrat de référence du kit **Open WebUI 0.9.6**. Décrit le **rituel comportemental**
> d'identité et les conventions communes aux 8 personas. Chaque persona est livré en
> `models/<persona>.json`, **importable** comme un **Model** (Workspace > Models). Le system
> prompt y est porté par `params.system`. Mapping modèle↔persona : `MODELES.md`. Installation
> pas-à-pas : `README.md`.
...
**Models Open WebUI**) exécute, dans un cadre strict : **on ne code jamais avant d'avoir cadré**.
Sous Open WebUI, le roster n'est pas un ensemble de sous-agents dispatchables : c'est une
**galerie de personas** — un Model = un persona, on joue **un rôle à la fois**, annoncé
explicitement (on choisit le Model dans le sélecteur de modèle du chat).
...
> Open WebUI n'offre **ni dispatch multi-agents natif, ni hook garde d'identité**. Le rituel
> ci-dessous est donc **purement comportemental** : il est porté par le `params.system` de chaque
> Model, pas par un mécanisme de la plateforme. Aucun watcher ne le vérifie — chaque persona
...
clôture de B, A ne place **aucune phrase dans sa voix**. Sous Open WebUI (pas de dispatch natif),
la chaîne est **narrative** : le décideur change de Model dans le sélecteur, ou un orchestrateur
cite — jamais un vrai routage automatique.
...
## Limites assumées (Open WebUI)

- **Aucune dépendance à un dispatch multi-agents natif** : un Model = un persona ; la chaîne de
  délégation est **narrative**, pas un routage.
- **Pas de hook garde** → le rituel d'identité est **comportemental** (porté par `params.system`).
- **Slash-commands `prompts/`** (`/cadrage`, `/revue`…) et **Tools / Functions / Pipelines**
  (Python) → **itération 2**, hors MVP.
...
La mécanique git/votre serveur git/états des lieux reste portée par les **scripts PowerShell** iakaframe
