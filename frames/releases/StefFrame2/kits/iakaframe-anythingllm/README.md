...
> `http://<ANYTHINGLLM_URL>`), et au moins un **fournisseur LLM** branché (cf. `MODELES.md`,
...
iakaframe (cadrage-avant-code, états des lieux, git/votre serveur git) reste portée par le dossier
`specs/` de ton projet réel et par les **scripts PowerShell** iakaframe (agnostiques de l'agent) :
