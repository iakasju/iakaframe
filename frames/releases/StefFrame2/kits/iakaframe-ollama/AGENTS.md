...
- Un **outil agentique** sur Ollama. Recommandé all-in-one : **OpenClaw** (openclaw.ai —
  open-source, local-first, exécution + orchestration + canal <CHAT> + skills). Sinon un harnais
...
votre serveur git (HTTP + token, jamais commité) ou git local. État des lieux régénéré à chaque
