...
description: Exécute le checkpoint « update iakaframe » — régénère l'état des lieux, fait un commit global atomique et pousse sur votre serveur git. Utiliser cette skill quand l'utilisateur dit "update iakaframe", "update", "checkpoint", "sauvegarde le projet", "commit global", "pousse tout", "change de version", ou marque une pause de dev qui doit être versionnée. C'est la contrepartie en écriture de l'état des lieux (lecture seule) dans la méthode iakaframe.
---
...
pousse sur votre serveur git. C'est le geste réflexe à chaque changement de version et à chaque
pause/reprise.
...
1. **Lancer le script** dans le répertoire du projet (il enchaîne snapshot + commit
   global + push) :
   ```powershell
   pwsh <IAKAFRAME_HOME>\iakaframe-update.ps1 -Reason version -Version v0.4.0 -Note "..."
   ```
...
2. **Si le script n'est pas disponible**, reproduire sa séquence à la main :
   ...
   pwsh <IAKAFRAME_HOME>\iakaframe-snapshot.ps1 -Reason version -Version v0.4.0
...
- Si le remote n'existe pas encore → brancher d'abord via la skill `iakaframe-git`.
- `update` sur un projet absent de votre serveur git **bascule en `init`** (auto-détection).
