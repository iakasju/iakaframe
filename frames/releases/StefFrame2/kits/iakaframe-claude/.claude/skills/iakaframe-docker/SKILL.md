...
- **Containers nommés/préfixés par projet** : `<projet>-dev-<service>` (ex.
  `<votre-projet>-dev-db`, `<votre-projet>-dev-api`).
...
- **Ports hôte distincts** : chaque projet **décale** ses ports pour ne jamais entrer en
  collision avec un autre (ex. Postgres : <votre-projet> `5432` / <autre-projet> `5433` /
  <projet-3> `5434`). Le port **interne** au container reste standard ; c'est le
