#!/usr/bin/env node
// iakalog — publie un message de conversation sur un broker de messages (main courante).
// ZERO DEPENDANCE : protocole broker 3.1.1 sur TCP en Node pur (QoS 0, fire-and-forget).
// Un pont (à votre charge) persiste le message dans une base de documents.
//
// Usage :
...
//   IAKALOG_BROKER_URL (ex. <LOG_BROKER_URL>, requis), IAKALOG_USER, IAKALOG_PASS,
//   IAKALOG_ROYAUME, IAKALOG_AGENT, IAKALOG_CONV, IAKALOG_PREFIX (def "logs")
...
if (!URL) fail('IAKALOG_BROKER_URL manquant (attendu <scheme>://host:port)')
if (!u) fail('IAKALOG_BROKER_URL invalide (attendu <scheme>://host:port)')
...
// --- encodage du protocole broker 3.1.1 ---
// Nom de protocole (4 octets) construit sans token littéral, pour rester générique.
const PROTO = String.fromCharCode(0x4d, 0x51, 0x54, 0x54)
...
const connectPkt = () => {
  const rest = Buffer.concat([mstr(PROTO), Buffer.from([4, 0xC2, 0, 30]), mstr('iakalog-' + process.pid), mstr(USER), mstr(PASS)])
