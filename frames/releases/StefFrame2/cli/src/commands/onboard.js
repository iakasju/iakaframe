...
// Mode chapeau : Odin (local + global) + dashboard portefeuille + scan + projets en attente.
function runUmbrella(root, values, node) {
  ...
  // [2/3] Dashboard portefeuille
  console.log('\n[2/3] Dashboard portefeuille');
  const fwRoot = frameworkRoot();
  const dashSrc = values['dashboard-source']
    || (fwRoot ? path.join(path.dirname(fwRoot), 'portefeuille-dashboard') : '');
  const dashDest = path.join(root, 'portefeuille-dashboard');
  let dashOk = false;
  ...
  console.log('\n[*] Projets du chapeau');
  const meta = ['portefeuille-dashboard', 'iakaframe'];
  const pending = [];
