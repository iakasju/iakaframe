...
## Action récurrente — mémoire humaine <DOC_TOOL>

En plus des guides, **tu portes la mémoire humaine du projet dans <DOC_TOOL>** (auto-hébergé sur
votre serveur git). Aux moments de documentation (changement de version, pause/reprise), tu publies les
**docs structurants** du projet dans <DOC_TOOL> via ta **skill-outil `iakaframe-humandoc`** :

```bash
# config par env (jamais en clair, jamais commité) :
#   DOC_URL, DOC_EMAIL, DOC_PASSWORD
node ~/.claude/skills/iakaframe-humandoc/humandoc.mjs --project <nom> --root <chemin-projet>
```
...
pas son remplaçant. Détail : skill `iakaframe-humandoc` + `methode-de-travail.md`.
...
- **Secrets <DOC_TOOL>** : uniquement via variables d'env ; jamais en clair, jamais commités.
