...
> extraite et généralisée à partir de plusieurs projets réels.
>
...
- **Isolation Docker par projet** : chaque projet tourne dans sa **propre stack
  ...
  pour ne jamais entrer en collision avec un autre, ex. <votre-projet> 5432 / <autre-projet>
  5433 / <projet-3> 5434). Jamais de partage de stack ni de ressources entre projets.
...
|---|---|---|---|---|
| 🦅 **Odin** | 🟡 | Super-agent **portefeuille** : switch d'équipe, démarrage projet, création d'équipe | Portefeuille (le seul sur `<IAKAFRAME_HOME>`) | `iakaframe-odin` |
...
| 🎭 **Loki** | 🟠 | Design : supports on-brand (catalogue de chartes `design-*/`) | Transverse | `iakaframe-design` |
...
🦅 Odin**, disponible en permanence, est le **seul agent affecté à `<IAKAFRAME_HOME>`** (la racine de
...
```
l'utilisateur → 🦅 Odin (portefeuille, <IAKAFRAME_HOME>) → 🛡️ Aragorn (par projet) → agents
```
...
**Réveil d'Odin (par défaut).** Au **premier appel d'Odin dans `<IAKAFRAME_HOME>`** (par session), avant
toute autre chose, il **régénère puis affiche le dashboard portefeuille** (<charte-defaut> dashboard)&nbsp;:
`pwsh <IAKAFRAME_HOME>\<charte-defaut>-dashboard\scan.ps1` réécrit `data/projects.js` (métriques git/LOC/tokens/état
des projets), puis il ouvre `<IAKAFRAME_HOME>\<charte-defaut>-dashboard\index.html`. Ensuite seulement il enchaîne
...
**Canal de communication — <CHAT> (bidirectionnel, via n8n).** Aragorn dialogue avec l'utilisateur
sur **<CHAT>**, piloté par n8n (qui porte les identifiants — aucun secret côté agent) :
sortant (états des phases, blocages, **demandes de feu vert**) et entrant (arbitrages, ordres
de dispatch, **feu vert prod** captés par un trigger n8n). <CHAT> devient un **canal de
...
- **`[ROYAUME]`** = le projet courant, en **MAJUSCULE** (ex. `MONPROJET`) ; pour 🦅 Odin =
  `PORTEFEUILLE`.
...
**Rendu.** Pastille emoji **partout** (terminal, <CHAT>, HTML) ; en session le libellé passe en
`code inline` pour ressortir ; en **HTML** il prend la **vraie couleur** de la phase. Exemples :

> 🔵 `[MONPROJET][Gandalf]` instruction prête à valider.
> 🔴 `[MONPROJET][Gimli]` dev en cours, commit `feat: …`.
> 🟢 `[MONPROJET][Gimli]` déployé en staging, `v0.6.0-rc1`.
> 🟣 `[MONPROJET][Helm]` prod en ligne, surveillance active.
...
`<IAKAFRAME_HOME>\.claude\settings.json` : **`SessionStart`** affiche le bandeau **IAKAFRAME** et
...
> Exemple réel (<votre-projet>) : l'exécution seule aurait embarqué FFmpeg pour le transcodage
...
- **Répertoire vide → nouveau projet.** Nom du dépôt = nom du dossier. On crée le
  dépôt votre serveur git, on déploie la structure, premier commit, état des lieux `v0.1.0`,
  push. Puis on remplit `CLAUDE.md` et `specs/PROJET.md`.
- **Répertoire avec déjà du dev → reprise.** On déploie la structure *autour* du
  code existant (rien d'écrasé), on branche votre serveur git si absent, on génère l'état des
  lieux de **reprise**, on en fait la synthèse et on propose la prochaine étape.

Orchestrateur : `iakaframe-onboard.ps1` (= `iakaframe-init` + `iakaframe-git` +
commit + `iakaframe-snapshot`). Un projet déjà doté d'un `CLAUDE.md` : celui-ci prime.

**Auto-détection init ↔ update.** Les deux commandes interrogent l'API votre serveur git : `init`
sur un dépôt déjà présent sur votre serveur git bascule en `update`, et `update` sur un dépôt
...
📁 PORTEFEUILLE           ← l'étage au-dessus des projets
    #odin                 ← saisie directe d'Odin (le décideur → runner)
📁 <PROJET>               ← UNE catégorie par projet
    #<projet>             ← canal unique du projet (mode absence : ses agents → le décideur)
```
...
## Git par défaut : votre serveur git (self-hosted)

Tout projet est versionné sur un **serveur git auto-hébergé (self-hosted)** —
...
### Mémoire humaine — documentation des fichiers importants dans <DOC_TOOL>

En plus de l'état des lieux (mémoire **de reprise**, dans le dépôt), on tient une **mémoire
humaine** des idées et projets dans **<DOC_TOOL>** auto-hébergé (instance
`notes.example.local`, hôte de votre choix). But : garder une **trace lisible, navigable et
...
à chaque **pause / reprise**. La publication <DOC_TOOL> **double** la régénération de la doc
...
**Comment (structure <DOC_TOOL>).** **Un espace par projet** (au nom du projet) → une **page
...
**Avec quoi.** Une skill dédiée `iakaframe-humandoc` (calquée sur
`iakaframe-log-conversation` : petit CLI Node, **config par variables d'env**, **aucun secret
en dépôt**). Elle s'authentifie à l'API <DOC_TOOL> (GoTrue email+mot de passe → token), provisionne
