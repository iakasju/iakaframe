...
- **Git** + un dépôt (votre serveur git recommandé en self-hosted ; un repo local suffit pour démarrer).
...
Remote type : `http://<git-host>:<port>/<user>/<repo>.git`, **HTTP + token**
(token jamais commité : variable d'env ou `.git/config` local). Adapter `<git-host>` à
