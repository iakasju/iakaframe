#!/usr/bin/env node
// iakaframe-humandoc — publie/rafraîchit la mémoire humaine d'un projet dans
// <DOC_TOOL> auto-hébergé (un ESPACE par projet, une vue d'ensemble + une sous-page par
// doc structurant). Idempotent, non destructif, défensif. ZÉRO dépendance (fetch natif).
//
// Usage :
//   node humandoc.mjs --project <nom> --root <chemin-projet>
//
// Identifiants résolus en cascade (aucun secret en dur, jamais commité) :
//   1. env d'abord : DOC_URL / DOC_EMAIL / DOC_PASSWORD
//   2. repli fichier dotenv local pour toute variable manquante :
//      $IAKAFRAME_DOC_ENV, sinon ~/.config/iakaframe/doc.env
//   (ex. DOC_URL=<DOC_URL>)
//
// Modèle de données (tranché avec le décideur, cf. specs/instructions/doc-doc-skill.md) :
...
// Bloc paragraphe <DOC_TOOL> (vide => delta vide accepté par l'API).
...
export function overviewBlocks(project, docNames, generatedAtIso) {
  return [
    para(`Projet : ${project}`),
    para(`Mémoire humaine <DOC_TOOL> générée le ${generatedAtIso}`),
    para(''),
...
// Chemin du fichier d'identifiants (override env, sinon ~/.config/iakaframe/doc.env).
export function docEnvPath(env) {
  if (env.IAKAFRAME_DOC_ENV) return env.IAKAFRAME_DOC_ENV
  return path.join(os.homedir(), '.config', 'iakaframe', 'doc.env')
}
...
export function resolveCredentials(env, fsApi = fs) {
  const KEYS = ['DOC_URL', 'DOC_EMAIL', 'DOC_PASSWORD']
  const creds = {}
  for (const k of KEYS) creds[k] = env[k]
  if (KEYS.some((k) => !creds[k])) {
    const fromFile = readEnvFile(docEnvPath(env), fsApi)
    for (const k of KEYS) if (!creds[k] && fromFile[k]) creds[k] = fromFile[k]
  }
  return {
    base: creds.DOC_URL,
    email: creds.DOC_EMAIL,
    password: creds.DOC_PASSWORD,
  }
}

// ───────────────────────── Client HTTP <DOC_TOOL> ─────────────────────────
...
class DocClient {
  ...
  async _req(method, p, body, _retried = false) {
    ...
    try {
      ...
      throw new DocError(`instance injoignable (${this.base}) : ${e.cause?.code || e.message}`)
    }
    ...
    if (r.status < 200 || r.status >= 300) {
      throw new DocError(`${method} ${p} -> HTTP ${r.status} ${(j.message || j._raw || '').slice(0, 160)}`)
    }
    return j
  }
  ...
  async auth() {
    ...
    if (!r.ok || !j.access_token) {
      throw new DocError(`authentification refusée (HTTP ${r.status} ${j.msg || j.error_code || ''})`)
    }
  ...
  async resolveWorkspace() {
    ...
    const ws = (j.data || [])[0]
    if (!ws) throw new DocError('aucun workspace accessible pour ce compte')
...
export async function run(argv, env) {
  ...
  if (!project || !root) {
    throw new DocError('usage : node humandoc.mjs --project <nom> --root <chemin-projet>')
  }
  const { base, email, password } = resolveCredentials(env)
  if (!base || !email || !password) {
    throw new DocError(
      `config manquante : définis DOC_URL/EMAIL/PASSWORD (env), ` +
      `ou renseigne ${docEnvPath(env)}`,
    )
  }
  if (!fs.existsSync(root)) {
    throw new DocError(`chemin projet introuvable : ${root}`)
  }

  const docs = resolveDocPaths(root)
  console.log(`doc-doc: projet "${project}" — ${docs.length} doc(s) structurant(s) trouvé(s)`)

  const client = new DocClient({ base, email, password })
  await client.auth()
  ...
  console.log(`doc-doc: espace "${project}" ${space.created ? 'créé' : 'réutilisé'} (${space.id.slice(0, 8)})`)
  ...
  console.log(`doc-doc: vue d'ensemble ${ov.replaced ? 'mise à jour' : 'créée'}`)
  ...
  console.log(`doc-doc: terminé — ${created} créé(s), ${updated} mis à jour, ${skipped} ignoré(s)`)
...
if (isMain) {
  run(process.argv.slice(2), process.env)
    ...
    .catch((e) => {
      ...
      console.error('doc-doc:', e.message)
      process.exit(1)
    })
}
