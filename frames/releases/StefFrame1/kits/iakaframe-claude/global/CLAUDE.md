...
> Lu dans **toutes** les sessions Claude Code du décideur.
> Référence complète : `<IAKAFRAME_HOME>\` (`methode-de-travail.md`, `kit-claude/`).

## Méthode de travail par défaut

le décideur travaille selon la méthode **iakaframe** : un décideur au-dessus d'une équipe
d'experts, périmètres étanches, zéro chevauchement.

- **Le décideur** = le décideur, l'utilisateur (vision, arbitrages, validation, test réel) —
...
> **Les deux commandes s'auto-détectent** (via l'API votre serveur git) : `init iakaframe` sur un
> dépôt **déjà présent sur votre serveur git** bascule en `update` ; `update iakaframe` sur un
> dépôt **absent de votre serveur git** (ou sans git local) bascule en `init`. On peut donc taper
> indifféremment l'une ou l'autre — le bon comportement est choisi automatiquement.

Quand le décideur dit **« init iakaframe »** (ou « initialise/lance iakaframe ») dans un
...
1. Créer le projet avec le **nom du répertoire** comme nom de dépôt :
   `pwsh <IAKAFRAME_HOME>\iakaframe-onboard.ps1` (structure + dépôt votre serveur git + 1er commit +
   état des lieux v0.1.0 + push). Token via `<GIT_TOKEN>`.
2. Remplir avec le décideur `CLAUDE.md` (stack, commandes, backlog) et `specs/PROJET.md`
   (vision, décisions).
...
1. **Ne rien écraser.** Déployer la structure autour du code existant + brancher
   votre serveur git si pas de remote : `pwsh <IAKAFRAME_HOME>\iakaframe-onboard.ps1` (init est
   non destructif ; si un `origin` existe déjà, garder celui-ci).
2. Générer l'état des lieux de **reprise** :
   `pwsh <IAKAFRAME_HOME>\iakaframe-snapshot.ps1 -Reason reprise`.
3. **Lire `specs/etat-des-lieux.md`**, en faire une synthèse au décideur (où on en est,
...
Quand le décideur dit **`iakastart`**, **`iakaframe`** ou **`odin`** — en **début** ou en
...
## Dépôt git par défaut : votre serveur git (self-hosted)

Remote par défaut de tout projet : **votre serveur git sur le LAN votre serveur git**,
...
`pwsh <IAKAFRAME_HOME>\iakaframe-snapshot.ps1 -Reason version|pause|reprise -Note "..."`.
...
Quand le décideur dit **« update iakaframe »** (ou « update » dans un projet de la
méthode) : lancer `pwsh <IAKAFRAME_HOME>\iakaframe-update.ps1` dans le répertoire. Ça
