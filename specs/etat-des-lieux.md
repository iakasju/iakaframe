# Etat des lieux - iakaframe

> Genere par iakaframe-snapshot.ps1 le 2026-06-11 23:41 (motif: version).
> A regenerer a chaque changement de version et a chaque pause/reprise.

## Etat courant

| Champ | Valeur |
|---|---|
| Version | v0.3.0 |
| Branche | main |
| Dernier commit | be3bf1b chore(iakaframe): update etat des lieux + commit global (version v0.2.0) |
| Arbre | MODIFICATIONS NON COMMITEES |
| Fichiers (hors .git/node_modules) | 20 |
| Note | doc HTML impactee : commandes init/update, Forgejo, cycle de doc, auto-detection |

## Commits recents

| Hash | Date | Sujet |
|---|---|---|
| `be3bf1b` | 2026-06-11 | chore(iakaframe): update etat des lieux + commit global (version v0.2.0) |
| `43b25c0` | 2026-06-11 | chore(iakaframe): update etat des lieux + commit global (manual) |
| `5028d0c` | 2026-06-11 | chore(iakaframe): update etat des lieux + commit global (version v0.1.0) |
| `91a195f` | 2026-06-11 | docs: etat des lieux initial (iakaframe-snapshot v0.1.0) |
| `3827dd6` | 2026-06-11 | chore: init iakaframe (methode, kit, scripts, doc iakabox) |

## Reprise du travail (a completer par Cowork)

- **Ce qui vient d'etre fait** : <!-- ... -->
- **En cours / a reprendre** : <!-- ... -->
- **Prochaine etape concrete** : <!-- premiere action a faire en reprenant -->
- **Pieges connus** : <!-- ... -->

## Journal (versions & pauses)

| Date | Motif | Version | Branche | Note |
|---|---|---|---|---|
| 2026-06-11 23:41 | version | v0.3.0 | main | doc HTML impactee : commandes init/update, Forgejo, cycle de doc, auto-detection |
| System.Object[] | System.Object[] | System.Object[] | System.Object[] | System.Object[] |

