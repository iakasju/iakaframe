...
| **`CRITERE-BACKLOG-D10`** | 🛑 **NEUF (post-gate) — reprendre le CRITÈRE, pas la liste** : le **volet skills** de `G-ROUTE-1` (`D8`) ne distingue pas un **chemin de fichier CITÉ** d'un **jeton de skill ATTRIBUÉ**. Lui apprendre cette distinction rendrait l'exemption **(c)** de `BACKLOG.md` (§ 13.0) **inutile** — elle **périrait alors d'elle-même** au sens de `D5`, et `CA-7` redescendrait de **4 à 3** exemptions de chemin | **Motif — la recommandation de 🏹 Legolas, citée telle quelle** : “`D10` prescrit littéralement *« si l'on doit ajouter des lignes, c'est que le critère est mauvais — on corrige le critère, pas la liste »*, et le remède doctrinalement propre existe : apprendre au volet skills à distinguer un **chemin de fichier cité** d'un **jeton de skill attribué**. Gimli a pris l'exemption de chemin plutôt que le critère, et c'est ce qui porte `CA-7` de 2 à 4. […] si tu reprends un des trois, prends celui-là.” — **Pourquoi successeur et NON reprise** : la coordination a **confirmé (c) en l'état** et **refusé la reprise à chaud**, au motif que rouvrir un lot gaté `PASS` pour améliorer **le plus faible de trois arbitrages défendables** affaiblit la valeur du gate — *un gate qui se rouvre pour du confort n'est plus un gate*. **Ce qui est nommé n'est pas perdu** : d'où cette entrée. **Provenance : arbitrage de COORDINATION sous autonomie déléguée — PAS du décideur, qui ne l'a pas énoncé — donc RÉVERSIBLE s'il le reprend**, au même titre que les quatre arbitrages **(a)**, **(b)**, **(c)** (§ 13.0) et **(d)** (§ 15.0). **Titulaire : 🧙 Gandalf** (geste de **critère**, donc de cadrage amont : définir ce que la garde doit distinguer — pas une retouche d'exécution) |
