...
| Version CLI documentée | `@naonedge/iakaframe` **v0.41.0** (source : `cli/package.json`) |
...
**Contrat de sortie machine C-JSON** (`cli/src/lib/output.js`, verrouillé par
`cli/test/guard-json-output.test.js`) : racine toujours un objet, `ok` en première clé, collection
⇒ tableau au pluriel + `count` frère, erreur ⇒ `{ ok:false, error, … }` sur stdout + exit 1 + stderr
vide, `--json` partout booléen. **Règle 6 — abstention légale** (`C-JSON-COUVERTURE-COMPLETE`, J0) :
`ok:false` avec **exit 0** n'est licite **que si** la charge porte un `status` non vide qui **nomme**
l'abstention (« je n'ai rien pu mesurer », ex. `vendor-check` sans frère GUI, `status:"skipped"`) —
jamais pour « j'ai mesuré et c'est mauvais » (qui reste la règle 4, exit 1). Un `ok:false` en exit 0
**sans** `status` n'est pas une abstention, c'est un défaut.
...
| `repo [<nom>]` | `--path <dir> --repo <nom> --provider <nom> (défaut forgejo) --create --description "ascii"` | Branche (ou **crée**, avec `--create`) le remote d'un dépôt git **EXISTANT** — geste **provider-neutre** (`--provider`, un seul adaptateur codé en MVP : `forgejo`). Sans `--create` : **test d'existence en lecture seule** + remote **local** seulement, **jamais** de création par effet de bord (refus explicite si le dépôt distant n'existe pas). `<nom>` (positionnel, alternative à `--repo`) : nom du dépôt distant (défaut : nom du dossier). **`--path <dir>`** : racine du dépôt git ciblé (défaut : dossier courant) — **pas** le dossier chapeau ni la racine de bibliothèque, distincts et sans option dédiée sur ce verbe : il opère sur un dépôt git précis. **`--description`** n'a de sens qu'avec `--create` (ASCII uniquement côté Forgejo). Précondition : pas un dépôt git → refus vers `onboard`. |
...
| `config` | `--path <dir> --runner claude-code\|ollama\|litellm\|codex --node <n> --aider-model <m> --root <chapeau> --json` | Écrit/màj `<projet>/iakaframe.json` (runner + nœud). **`--root <chapeau>`** : dossier chapeau (défaut `~/work`) où chercher un build local d'iakaIDE pour le diagnostic — **pas** la bibliothèque (sens *chapeau*, cf. avertissement `--root` ci-dessous). **`--json`** : sortie C-JSON `{ ok, path, runner, node, diagnostics, warnings }` (honoré et testé depuis l'origine — `--json` manquait ici et au registre, écart fermé par `C-JSON-COUVERTURE-COMPLETE`, J0). |
| `install` | `--dry-run --yes --root <dir> --hosts a,b --target-claude <dir> --backup-dir <dir> --apps-dir <dir> --json --events --feu-vert refus\|stdin` | **Contrat machine** (`contrat-machine-du-verbe-install.md`) : trois canaux **machine** s'ajoutent au mode humain, **sans changer aucune ligne de prose** (CA-M8, verbe gaté PASS). **`--json`** entre désormais au contrat **C-JSON** — il **bufferise** les événements et imprime **une seule racine objet** `{ ok, count, evenements[], etatAtteint, reprise }` (`{ ok:false, error, … }` sur stdout, exit 1, rien sur stderr en échec) — avant ce lot, `--json` imprimait de la prose et sortait en 0, un défaut désormais soldé. **`--events`** diffuse un flux **NDJSON** (contrat **C-EVT**, une ligne = un objet compact) sur stdout : annonce de chaque étape (quoi/où/version/fusion/source retenue/sources consultées), demande et octroi de feu vert, provenance en champs, rapport de rollback, état atteint et commande de reprise — les deux sous-processus délégués (mise à jour npm, `install.mjs`) y sont **capturés** (`stdio:['ignore','pipe','pipe']`) et leur sortie **ré-émise** en `log-delegue`, jamais mêlée au flux. **`--feu-vert <mode>`** choisit le canal de consentement machine : `refus` (**défaut**, AR-4 tenu : toute demande non couverte par `--yes` ou un terminal interactif est refusée) ou `stdin` (lit **une ligne** de réponse par étape, forme `{"etape":n,"reponse":"oui"|"non"}` ou `oui`/`non` nue — une réponse **hors séquence** ou illisible est **toujours un refus**, jamais un feu vert supposé ; zéro timeout, EOF = fin). **Combinaisons refusées explicitement** (jamais dégradées) : `--json --events` et `--json --feu-vert stdin` (le premier bufferise, les deux autres exigent un flux immédiat). En mode humain (aucun des trois drapeaux), **rien ne change** — même chaîne, mêmes messages, mêmes 4 étapes ci-dessous. **`etatAtteint.etapesFaites` en `--dry-run`** : reste **vide pour les quatre étapes, symétriquement** — une étape *décrite* en dry-run n'a rien écrit, elle ne compte donc jamais comme faite, quel que soit son `etat` d'`etape-terminee` (`'dry-run'` ou `'sautee'`), distinct de `'faite'`.
<br><br>**Chaîne complète** (`chaine-complete-install-amorcage-dmg-msi.md`, lots A+C.1) : annonce **toujours** les **4 étapes / 3 téléchargements** (AR-A : CLI, méthode, IakaCockpit, iakaFrameGUI) et **joue les 4**. **Étape 1 (CLI)** : sens **unique** ici — *mise à jour* (AR-G : « le CLI ne peut pas s'installer lui-même ») — compare la version courante à un **réservoir vivant** local (`--root`, sinon `<chapeau>/iakaframe`) puis, à défaut, aux **sources réseau ordonnées** d'AR-H (tarball GitHub = voie publique, registre npm NAS = voie LAN), et **nomme** la source retenue. **Étape 2 (méthode)** *(BUNDLE-INSTALL-MJS-ABSENT, 2026-09-04 : `install.mjs` voyage désormais avec le paquet publié, `cli/_bundled/install.mjs`)* : **délègue** à `install.mjs` (M4, jamais réimplémenté) trouvé dans le réservoir **DÉSIGNÉ par l'étape 1** (AR-I(a)) — le **vivant** si le vivant gagne, l'**embarqué** (`_bundled/`) si l'embarqué gagne (vivant absent OU vivant plus ancien) ; refuse explicitement, en **nommant les deux chemins cherchés** (vivant candidat et embarqué), si **ni l'un ni l'autre** ne porte `install.mjs` — bundle incomplet, **CA-21′** (rectification datée de CA-21), refus qui nomme la cause, jamais un succès silencieux. **Étapes 3/4 (IakaCockpit, iakaFrameGUI)** — **macOS, Linux et Windows couverts** (lots C.1, `ETAPES-3-4-WINDOWS-LINUX`/W-L puis /W-W, 2026-09-05) ; seuls `win32/arm64`, `linux/arm64` et `darwin/ia32` restent refusés explicitement (CA-15), jamais une simulation. Résout le manifeste de l'app via ses **deux endpoints déjà décidés** par l'app elle-même (M10 : NAS Forgejo puis GitHub raw), sélectionne l'entrée **clé d'installeur d'abord, générique en repli** (M6 — sur Linux, `linux-x86_64-appimage` puis `linux-x86_64` ; sur Windows, `windows-x86_64-nsis` puis `windows-x86_64` ; **jamais** de repli sur `linux-x86_64-deb`/`-rpm` ni sur `windows-x86_64-msi`, exclus doctrinalement — root/UAC requis, rollback non honnête, AR-W1(a)/AR-W2(a)), **vérifie la signature minisign** du bundle **avant** toute écriture (CA-14, refus si invalide, sur les trois plateformes). **Sur macOS** : pose le `.app.tar.gz` dans `--apps-dir` (défaut `~/Applications`), inchangé. **Sur Linux** : pose l'**AppImage** — copie de l'octet vérifié + `chmod +x` — sous `<apps-dir>/<Nom>.AppImage` (même drapeau, même défaut qu'macOS, aucun `.desktop` écrit — successeur `LANCEUR-DESKTOP-LINUX`) ; le lancement réel de l'AppImage sur une distribution (avec ou sans FUSE 2) reste un **gate humain**, jamais compté couvert. **Sur Windows** : `--apps-dir` **n'a aucun effet** (E-4, annoncé explicitement à chaque étape, CA-W9) — exécute l'installeur `.exe` NSIS `currentUser` en silencieux (`/S`, **jamais** `/R`, AR-W1(a) : pas d'UAC), **jamais le `.msi`** (`perMachine`, UAC obligatoire, exclu). Avant d'écrire, **lit le registre de désinstallation** (`HKCU\...\Uninstall\<productName>`, AR-5 § 2.3) : une version installée dont le dossier (`InstallLocation`) est **indéterminable** fait **REFUSER d'écrire** (garde 1) ; une version au dossier connu est **sauvegardée** avant relance de l'installeur ; rien d'installé ⇒ pose neuve, rollback = **`uninstall.exe /S _?=<InstallLocation>`** découvert **après coup** par une seconde lecture du registre. **`_?=` est OBLIGATOIRE** (reprise AR-W5(a), précision uninstall synchrone, 2026-09-06, après la première mesure réelle du banc CI Windows, run `33997947501`) : sans lui, le désinstalleur NSIS **se copie dans `%TEMP%` et rend la main immédiatement** (doc NSIS officielle, *Command Line Parameters*) — un code de sortie `0` n'atteste alors QUE le lancement de la copie, jamais la fin réelle de la désinstallation, précisément le défaut mesuré en CI. `_?=<InstallLocation>` force l'exécution **en place et synchrone** ; le module **relit ensuite le registre** et ne déclare `defait:true` **que si la sous-clé a réellement disparu** (relecture bornée à 10 tentatives espacées en défense), sinon `ok:false` avec une désinstallation **NON confirmée** nommée comme telle. `_?=` empêchant aussi le désinstalleur de s'auto-supprimer (même doc, *Uninstall Section* : l'auto-suppression ne fonctionne que depuis une copie temporaire), un **nettoyage best-effort** du dossier et de `uninstall.exe` suit la confirmation, et un échec de ce nettoyage est **énoncé** (« résidu non nettoyé »), jamais masqué ni bloquant pour le verdict de désinstallation déjà confirmée — dans tous les cas, le rapport de rollback **énonce** en plus que le registre (valeurs) et les raccourcis ne sont pas rétablis (garde 3, résidu jamais tu). **Si le rollback ne peut même pas identifier la cible** (pose jamais complétée, ou `InstallLocation` introuvable après une pose pourtant réussie), il ne tente **ni** suppression **ni** désinstallation à l'aveugle : il **énonce nommément** ce résidu (fichiers éventuellement posés, clé de registre, raccourcis) et la reprise manuelle (rechercher l'application dans « Applications et fonctionnalités », ou son `uninstall.exe` sous `%LOCALAPPDATA%`) — jamais une exception brute (garde 3, reprise post-gate du 2026-09-06). L'installation réelle, l'absence d'UAC et le rollback réel restent un **gate humain** sur machine Windows, jamais compté couvert. **Rollback automatique (AR-5, trois gardes)**, identique sur les trois plateformes (cible remplaçable dans son entier — fichier, dossier, ou dossier découvert par le registre), si l'étape 4 échoue après que l'étape 3 a écrit : (1) ne défait que ce qu'une **sauvegarde horodatée prouve** avoir changé, **refuse de dérouler** si elle manque ; (2) une app **déjà présente** avant la chaîne est **restaurée**, jamais effacée (ou **désinstallée** via son propre désinstalleur sur Windows si rien n'existait avant) ; (3) énonce ce qu'il **a su** défaire et ce qu'il **n'a pas su**, jamais un « restauré » global. **Réservoir (AR-F)** : « le plus récent gagne ; à égalité, le vivant » — l'égalité est le **cas nominal** (poste où le CLI tourne depuis sa propre source), jamais un `else`. **Provenance obligatoire** sur chaque étape 1, format imposé (`réservoir : vivant <chemin> (vX) — embarqué vY, <raison>`). **AR-4** : chaque étape s'annonce (quoi/où/version/fusion) puis attend un feu vert ; `--yes` les saute **toutes**, sans exception partielle. **`--dry-run`** ne modifie rien (empreinte disque avant/après identique). **Garde AR-1/AR-4 (§5.5)** : pendant la chaîne, le déploiement automatique du kit hôte (AR-1) reste **désarmé** jusqu'à validation explicite de l'étape 2 — jamais un court-circuit silencieux. |
...
| `go <projet>` | `--path <dir> --project <nom> --runner <r> --do "tache" --root <chapeau>` | Lance l'action du projet via son runner (`claude-code\|ollama\|litellm\|codex` ; launchers legacy : `aider`, `iakaide`). **`--project <nom>`** : alias du positionnel `<projet>` — un **nom** de projet résolu sous le chapeau, **pas** un dossier (sens distinct du `--project <dir>` d'`agents`/`skills`/`produit`/`open`). **`--root <chapeau>`** : dossier chapeau sous lequel résoudre `<projet>` (défaut `~/work`), sens *chapeau* (cf. avertissement `--root` ci-dessous) — sans effet avec `--path`. |
| `agents` | `list \| affect \| fullteam \| status` · `--agent <nom> --project <dir> --global --force --json` | Équipe de personas : inventaire / affectation / équipe complète / statut. **`--json`** (sur `list`/`status`) : sortie C-JSON. |
| `skills` \| `skills deploy` | `--project <dir> --global --check --json` | Déploie l'union résolue des skills de la team active vers `<cible>/.claude/skills/` (non destructif) ; `--check` échoue sur dérive (calque `agents generate`). **`--json`** : `{ ok, count, skills:[{skill,status}], orphans, target, drift }`. |
...
| `brief <projet>` | `--path <dir> --project <nom> --root <chapeau> --font <nom>` | Entrée projet : titre + tableau (dernière étape + backlog) + agents. **`--project <nom>`** : alias du positionnel `<projet>` — un **nom** de projet, pas un dossier (même divergence de sens que sur `go`). **`--root <chapeau>`** : dossier chapeau sous lequel résoudre `<projet>` (défaut `~/work`), sens *chapeau* — sans effet avec `--path`. |
| `recap <projet>` | `--path <dir> --project <nom> --root <chapeau> --n <nb commits>` | Fermeture : tableau récap de session (commits + agents + projet). **`--project <nom>`** : alias du positionnel `<projet>` — un **nom**, pas un dossier (même divergence de sens que sur `go`). **`--root <chapeau>`** : dossier chapeau sous lequel résoudre `<projet>` (défaut `~/work`), sens *chapeau* — sans effet avec `--path`. |
...
| `add <kind> <fic>` | `kind : team\|method\|binding` · `--root --force --guide --json` | Livre un assemblage (valide les réfs I1). `--guide` (Lot A) : propose `kind`, puis demande l'id/fichier en texte libre (aucune autorité possible sur une cible neuve). |
...
| `attach <skill>` | `--persona <id>` · `--root --force --guide --json` | Attache un skill à un persona : **mute le seul `skills:[]`** du frontmatter (source unique de vérité) ; refuse un skill absent de la bibliothèque (I1) sauf `--force`. Le `+` symétrique de `detach`. `--guide` (Lot A) : propose skill puis persona. |
| `detach <skill>` | `--persona <id>` · `--root --guide --json` | Détache un skill d'un persona : retire l'id de `skills:[]` (idempotent, réversible par `attach`). Le **`−` au « titre du skill »** — affordance rendue par la vue, jamais écrite dans le corps du persona. `--guide` (Lot A) : propose persona puis un skill **attaché** à cette persona. |
| `assemble <m> <t>` | `--write --binding --root --json` | Compose un kit (méthode + team [+ binding]) — dry-run par défaut. **`--root <dir>`** : racine de bibliothèque (sens *bibliothèque*, cf. avertissement `--root` ci-dessous). |
| `switch` \| `use <m> <t>` | `--path --binding --rollback --root --guide --json` | Bascule un projet vers une méthode/team. (`use` = alias de `switch`.) **`--root <dir>`** : racine de bibliothèque (sens *bibliothèque*). `--guide` (Lot A) : propose méthode puis team — **sans effet** si `--rollback` est demandé (jamais deviné), ne propose jamais `--force`. |
