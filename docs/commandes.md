...
| `go <projet>` | `--path <dir> --runner <r> --do "tache" --root <chapeau>` | Lance l'action du projet via son runner (`claude-code\|ollama\|litellm\|codex` ; launchers legacy : `aider`, `iakaide`). **`--root <chapeau>`** : dossier chapeau sous lequel résoudre `<projet>` (défaut `~/work`), sens *chapeau* (cf. avertissement `--root` ci-dessous) — sans effet avec `--path`. |
...
| `brief <projet>` | `--path <dir> --root <chapeau> --font <nom>` | Entrée projet : titre + tableau (dernière étape + backlog) + agents. **`--root <chapeau>`** : dossier chapeau sous lequel résoudre `<projet>` (défaut `~/work`), sens *chapeau* — sans effet avec `--path`. |
| `recap <projet>` | `--path <dir> --root <chapeau> --n <nb commits>` | Fermeture : tableau récap de session (commits + agents + projet). **`--root <chapeau>`** : dossier chapeau sous lequel résoudre `<projet>` (défaut `~/work`), sens *chapeau* — sans effet avec `--path`. |
