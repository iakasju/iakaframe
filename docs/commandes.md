...
| `models` | `--json --hosts a,b,c --timeout <sec> --path <projet> --root <dir>` | **Modèles d'IA suggérés par `roleKey`** et leur mise à disposition. Sans option : **process interactif** en 4 temps — état des lieux (suggéré / affecté / disponible, avec la **date de fraîcheur** des suggestions) → « voir les suggestions ? » → diff → **installer / remplacer / retirer sur validation explicite**. Cinq cibles : `ollama-local`, `ollama-distant`, `litellm`, `claude`, `codex` (pour les deux derniers, « installer » = **vérifier**, rien à télécharger). Source unique des suggestions : `models/suggestions.json`. **Aucun téléchargement ni écriture sans gate** ; `--json` et l'absence de terminal s'arrêtent à l'état des lieux. Hôtes/ports neutres (`IAKAFRAME_HOSTS`, `IAKAFRAME_OLLAMA_PORT`, `IAKAFRAME_LITELLM_PORT`, `IAKAFRAME_LITELLM_KEY`). ⚠️ Ici « model » = **modèle d'IA**, pas modèle de frame. |
