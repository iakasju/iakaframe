...
| `config` | `--path <dir> --runner claude-code\|ollama\|litellm\|codex --node <n> --aider-model <m> --json` | Écrit/màj `<projet>/iakaframe.json` (runner + nœud). **`--json`** : sortie C-JSON `{ ok, path, runner, node, diagnostics, warnings }` (honoré et testé depuis l'origine — `--json` manquait ici et au registre, écart fermé par `C-JSON-COUVERTURE-COMPLETE`, J0). |
